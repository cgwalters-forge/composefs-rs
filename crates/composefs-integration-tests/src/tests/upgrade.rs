...
/// Content of a large file, varied by `seed` so each image has its own
/// objects.
fn large_file(seed: u8) -> Vec<u8> {
    (0..LARGE_FILE_SIZE)
...
fn create_layout(parent: &Path, fixture: &Fixture) -> Result<PathBuf> {
    ...
    let large = large_file(fixture.seed);
    for (name, data) in [
        ("usr/small.txt", small.as_bytes()),
        ("usr/large.bin", &large),
    ] {
        let mut header = tar::Header::new_gnu();
...
/// Asserts that `fsck` and `oci fsck` find no problems.
fn assert_fsck_ok(sh: &Shell, cfsctl: &Path, repo: &Path) -> Result<()> {
    for fsck in [&["fsck"][..], &["oci", "fsck"]] {
        let output = cmd!(sh, "{cfsctl} --repo {repo} {fsck...} --json").read()?;
